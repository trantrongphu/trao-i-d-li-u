...
void call_AVG()
{
  if (digitalRead(button_call_AVG)==0)
  {
    ...
    {
      mqtt_client.publish(topic_publish,"1");
    }
  }
}
void line_1()
{
  if (digitalRead(button_line_1)==0)
  {
    while (digitalRead(button_line_1)==0)
    {
      ...
      if (digitalRead(button_line_2)==0)
      {
        ...
        if (mqtt_client.connected())
        {
          ...
          // String payload = "";
          // payload += client2;
          String payload = "{\"client2_line2\":";
          payload += client2_line2;
          payload += "}\0";
    ...
    if (mqtt_client.connected())
    {
      ...
      // String payload = "";
      // payload += client2;
      String payload = "{\"client2_line1\":";
      payload += client2_line1;
      payload += "}\0";
      mqtt_client.publish(topic_publish, payload.c_str());
    }
  }
...
void setup() 
{
  ...
  mqtt_client.publish(topic_publish," client 2 hello Server");
